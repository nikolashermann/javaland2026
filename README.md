...
To rotate the client secret, you need to regenerate the secret in Keycloak and then restart the service:

1. **Regenerate the secret** via Terraform:
   ```bash
   cd terraform
...
2. **Restart the `api-client`** to pick up the new secret:
   ```bash
   docker compose up -d --force-recreate api-client
   ```
*Note: In a production environment, you would typically use secrets management that allows for rotation without downtime, but here we restart the container to reload the secret file.*
